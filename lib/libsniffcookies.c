...
void got_packet(u_char *args, const struct pcap_pkthdr *header,
  ...
  http_payload_addr =
      (void *)(packet + ETHER_HDR_LENGTH + IP_HDR_LENGTH + tcp_header_size);
  http_payload_size =
      header->len - (ETHER_HDR_LENGTH + IP_HDR_LENGTH + tcp_header_size);
...
void display_csv_data(Host_cookies *host_cookies) {
  ...
  printf("%s;%s;%s;%s", host_cookies->host_dst, host_cookies->ip_src,
         host_cookies->resource, host_cookies->request_type);
